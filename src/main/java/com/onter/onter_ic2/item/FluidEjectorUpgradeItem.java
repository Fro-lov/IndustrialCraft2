...
public class FluidEjectorUpgradeItem extends UpgradeItem {
    public FluidEjectorUpgradeItem(Properties properties) {
        super(UpgradeType.FLUID_EJECTOR, properties);
    }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (player != null && context.getLevel().isClientSide) {
            player.displayClientMessage(Component.literal("§a[IC2] Жидкостный выталкиватель настроен на: §e" + getDirName(clickedFace)), true);
        }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (level.isClientSide) {
            player.displayClientMessage(Component.literal("§a[IC2] Жидкостный выталкиватель настроен на: §e" + getDirName(next)), true);
        }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        tooltipComponents.add(Component.literal("§6Улучшение «Жидкостный выталкиватель»"));
        tooltipComponents.add(Component.literal("§7Автоматически выгружает жидкость в соседний резервуар/трубу."));
